...
- **Per-slide captions: REMOVED, not deferred (2026-08-07).** The admin caption input and the entire app-side caption path are deleted. Three reasons: the design export has no caption pattern at all — its slide model is `label` / `alt` / `key` (`docs/design-source/redesign-2026-08/template.extracted.html:316-320`) — so drawing one would require inventing new design under CONSTRAINT-05; the T46 rewrite from embla to `ProjectFrame` had already silently dropped the render; and `project_media` holds zero rows in production, so nothing is lost. The `project_media.caption` column and the `010a` RPC are left in place. No migration.
- **Three accessibility regressions, none tracked as an open task (as of 2026-08-07).** These are not unshipped nice-to-haves: all three shipped under the embla carousel (T43.G) and the T46 rewrite to `ProjectFrame.tsx` dropped them. (1) **No keyboard ←/→** — the component registers no keydown handler at all, so tabbing to the arrow buttons is the only keyboard path. (2) **No `aria-live` slide announcement** — `aria-live` appears zero times across `app/` and `components/`, so slide changes are silent to screen readers; there is likewise no `useId` / `aria-controls` / `aria-labelledby` wiring in any public component, so the dots are not programmatically tied to the track. (3) **No `prefers-reduced-motion` rule** — the string appears nowhere in the repo, so `.sb-track`'s `.4s` transform (`app/styles/public-projects.css:41-45`) runs unconditionally. The fourth thing T43.G lost, boundary-disabled arrows, is not a regression to fix: `go()` wraps modulo `slides.length` and looping is the intended behaviour per the wrap G/W/T above.
