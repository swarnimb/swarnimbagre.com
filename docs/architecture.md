...
Neither T41 page invents a design value: both are composed entirely from classes that already exist in the export (`.container`, `SiteHeader`, `.title-block`, `.page-title`, `.page-lede`, plus the export's off-home action pill `.sb-actions` / `.sb-action` / `--primary` / `--secondary`, export `:358-367`). The export contains no 404 and no error page, so both are new compositions; `@designer` signed them off under CONSTRAINT-05 on 2026-08-07. **Home's `.h-actions` / `.h-btn` are not used here and never leave the home page** — they carry no `min-height` (~31.5px on mobile), which is wrong for the one control a dead-end page exists to offer, where `.sb-action` is 44px / 42px. Standing rule: system pages (404, error, any future `loading.tsx`) compose the shared shell plus `.sb-action`. See CONSTRAINT-05 for the one CSS line added under that sign-off (`cursor: pointer` on `.sb-action`).
