...
- **Server Action surface (F-14, audit pass 4):** every export of a `'use server'` module is a public Server Action with a stable hashed ID in the client bundle. After every build, `.next/server/server-reference-manifest.json` must list exactly the actions named in the test allowlist at `tests/server-actions-manifest.test.ts` — no more, no fewer. As of T22 the allowlist is `signInWithMagicLink` (T17), `signOut` (T18), `createProject` (T21), `updateProject` (T21), `deleteProject` (T22) — five IDs total. Any PR that adds a new action ID without updating the test allowlist + auditing the new surface is a wire-level enumeration regression. See §6.6.1, §6.6.6, `docs/auth-flow.md` §2a point 4, `docs/security-report.md` audit-4 F-14 and audit-5 F-14a/c/d.
