...
- [ ] Run the full Playwright suite (`admin-smoke.spec.ts`, `pages.spec.ts`, `admin-font.spec.ts`) against a live Supabase fixture with an authenticated session. **Blocks the next deploy.** Needs an environment, not a decision — an agent can close this once the fixture and session exist.
