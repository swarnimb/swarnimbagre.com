...
- [x] Screen-reader live region announces "Slide N of M, [alt text]" when active slide changes. — ~~**T46 regression:** no `aria-live` region exists anywhere under `app/` or `components/`; slide changes are silent to screen readers.~~ **Fixed 2026-08-11 (session 58).** `ProjectFrame.tsx` renders `<span className="sb-live" aria-live="polite" aria-atomic="true">` inside the existing `multi &&` block, announcing `Image N of M: {alt}` — phrasing per the CONSTRAINT-13 AT sub-rule, not the "Slide N of M" written here. Clipped by the new `.sb-live` rule in `public-projects.css` (`clip-path: inset(50%)`, not `display: none`, which would drop it from the accessibility tree). Covered by two render cases in `tests/ProjectFrame.test.tsx` (renamed from `.test.ts`).
- [x] `prefers-reduced-motion: reduce` honored: embla `duration: 0` when the media query matches. — ~~**T46 regression:** `prefers-reduced-motion` appears in no CSS file or component in the repo, so the `.4s cubic-bezier(.4, 0, .2, 1)` `.sb-track` transform runs unconditionally.~~ **Fixed 2026-08-11 (session 58)**, by CSS rather than the embla option this line assumes — embla is gone. `@media (prefers-reduced-motion: reduce) { .sb-track { transition: none; } }` sits directly under `.sb-track` in `public-projects.css`; paging still works, the slide simply arrives instead of travelling. Scoped to the track alone: the three remaining public transitions are colour/opacity at 0.18s. Not a CONSTRAINT-05 breakpoint — that rule governs width queries.
...
- [~] Multi-instance DOM ID hygiene: `React.useId()` for the `aria-controls` / `aria-labelledby` / dot button IDs. — **T46 regression:** `ProjectFrame.tsx` calls no `React.useId()` and emits no `aria-controls` / `aria-labelledby`; arrows and dots carry `aria-label` only, so the dots are not programmatically tied to the track. — **WON'T FIX, builder's decision 2026-08-11 (session 58),** on a recommendation to drop it. The dot and arrow labels already name the project (`Go to image N of {title}`), so a screen-reader user is never lost as to which card's carousel they are in; the linkage would make the widget more navigable, not unbreak it. The `useId()` half is not a live defect — no IDs are emitted, so nothing can collide across the stacked carousels on `/projects`; it only becomes necessary if the linkage is added. Re-raise only if the carousel gains real tab-panel semantics.
...
- [~] Arrow + dot button labels are typographic glyphs only (`←`, `→`, `•`) — CONSTRAINT-13. ARIA labels: `aria-label="Slide 1"` etc. — **Drifted at T46, and the drift is now the spec.** Arrows render `&lsaquo;` / `&rsaquo;` and dots are unlabelled CSS shapes, while the ARIA strings became prose (`Previous image of {title}`, `Go to image N of {title}`) — the exact phrasing T43.A rejected. **Resolved 2026-08-11 (session 58) in favour of the shipped code: the T43.A `Slide 1` wording is retired, not restored.** Several carousels stack on `/projects`, and `Slide 1` alone does not say which project is being paged; the terseness was style bought at the cost of the only job those strings have. Recorded as the CONSTRAINT-13 assistive-technology sub-rule — voice governs what is seen, AT-only strings are unambiguous first, and the emoji and em-dash bans still bind them. Visible chrome holds CONSTRAINT-13 as before, unchanged.
