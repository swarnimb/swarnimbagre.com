 ...
 * (T18) layered on top of the T10 device-variant header. Runs in the `node`
 * env so `Headers` / `Request` / `Response` match Next's middleware internals
 * (JSDOM polyfills fail the `instanceof Headers` check inside
 * `NextResponse.next({ request })`). Mocks `@supabase/ssr::createServerClient`
 * directly (the middleware imports the SSR helper at the module boundary).
 *
 ...
 * 'present' = non-null session, 'none' = null + no error, 'error' = AuthError,
 * 'throw' = `getSession` rejects.
 */
function mockSupabaseSession(
      ...
      return vi.fn().mockResolvedValue({
        data: {
          session: {
            ...
            user: { id: 'user-1', email: 'allowed@example.test' },
          },
        },
        error: null,
      });
  ...
  const stub = { auth: { getSession: make() } };
...
describe('admin gate — functional (F1-F4)', () => {
  ...
  it('F4: GET /admin/posts with an error from getSession redirects + logs "session expired"', async () => {
    mockSupabaseSession('error');
...
describe('admin gate — log-payload security (L1)', () => {
  /**
   * L1 — SEC-05 at the gate. Captures every log payload across all three
   * redirect outcomes; asserts the aggregate string contains no token-shaped
   * substring (access_token / refresh_token / sb- cookie prefix) and no
   * email-shaped string.
   */
  it('L1: no token-shaped strings appear in any console.info or console.error call across B/C/D', async () => {
    for (const behavior of ['none', 'error', 'throw'] as const) {
      mockSupabaseSession(behavior);
